...
private fun solve(input: String, parameters: Map<String, String>? = null): String {
    val ducksPerColumn = input.lines().map { it.toLong() }
    val duckComparatorIndices = (0..ducksPerColumn.size-1).zipWithNext()

    var roundsUsedUntilBalanced = 0L
    ...
    val predictedResultColumnSize = ducksPerColumn.sum().div(ducksPerColumn.size.toLong())
    val predictedResult = resultPhase1.map { predictedResultColumnSize - it }.filter { it > 0 }.sum()

    return (predictedResult + roundsUsedUntilBalanced).toString()
}
